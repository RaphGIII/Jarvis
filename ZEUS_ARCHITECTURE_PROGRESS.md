...
## Phase 30 — product hardening & semantic integrity (2026-08-27 evening) — LIVE VERIFIED

Baseline protected as tag `zeus-baseline-hardening-20260827` (= `b72c18e`, HEAD = origin = known-good at the start).

**Wake word (owner "Zeus").** Five root causes, all reproduced before fixing: `VoiceSettings` had no `wake_sensitivity`/`volume` fields (Save dropped both; the listener ran the manifest's 0.7 while the form showed 0.55); the Voice Studio test divided int16 PCM by 32768 and the detector cast the float back to int16 (near-silence — the 0.000 … 0.99 scores were noise, unrelated to the settings); `ZeusDetector.reset()` kept the wall-clock cooldown (an offline evaluation reported 7/30 for a model that fires on 27/30); training clips were padded with exact zeros and the model scored 0.99 on a silent recording at the zero transition; the live listener from 16:14 never reloaded the model trained at 19:27. Now one pipeline: `VoiceSettings` validates and persists every field (`data/jarvis/voice/settings.json`; `WAKE_INPUTS` excludes `volume`, pinned by test); `speech.wake_zeus.to_int16_frame` is the one PCM contract, `speech.wake_eval.score_wav` the one scorer for test, calibration and trainer; `resolve_threshold(owner, manifest)` gives one effective threshold with its source; the listener polls `/api/voice/wake` every 5 s, applies the threshold, hot-reloads a retrained model and reports fingerprint + threshold back (`listener_match`); TTS volume acts only in `ui/voice/playback.js`. Trainer: dither floor, silent owner recordings skipped (`owner_016.wav`), `--owner-holdout N`, manifest threshold from the owner evaluation. Evidence: hold-out recipe validation 10/10 positives (peaks ≥ 0.9999), 0/3 false (`V1_wake_holdout_validation.json`); shipping model `1b540fcadd731255` in-sample at the owner's 0.55: recall 26/29, 0/10 false, negatives max 0.018. Live: `/api/voice/wake/test` on `owner_002.wav` → 1.000 DETECTED at 0.55 (owner), identical at volume 0.1; negative → 0.018. Hard negatives (Jesus, Servus) are not recorded and therefore not reported. **The spoken microphone test is the owner's; not claimed here.**

**Ambient speech.** `core.hear()` now has an utterance gate: audio without a listening session (wake score from the listener via `X-Jarvis-Wake`, or `origin=ui` from the microphone button) is transcribed and refused; fragments ("Toys.", "So is...", one word repeated), trailing-off transcripts, low confidence and duplicates inside 12 s create no message, receipt or mission — each refusal is a DIAGNOSTIC event. Test I of the acceptance suite plays the live "Toys." sequence.

**Knowledge.** The owner-facing routes read `state/knowledge/graph.db` (empty) while the machinery wrote `palace.sqlite` (144 nodes) — the honest "0 nodes" that led the planner to a forbidden note file. One graph now (`core.graph`, legacy file migrated once), typed primitives `knowledge.create` (node + typed relations, verified by read-back and search), `knowledge.link`, `knowledge.read`, backlinks, delete, stats; node types extended (technical_finding, document, mission, owner_correction, verified_lesson, device), relations `applies_to`, `concerns`, `resolved_by`. Live: finding `29a0fd46260b4c0b` created with part_of→ZEUS, applies_to→Voice, concerns→Wakeword; found by search; backlink from Wakeword; the graph survived the restart.

**Planner semantics.** `PlanConstraints` extracted deterministically from the owner's sentence (negations → forbidden steps; "no fallback" → nothing else may be produced; outcome words → required outcome) and enforced in `Composer.parse` (a forbidden step gets `status=forbidden` and can never run). Step roles (required / optional / alternative / verification; observations optional by default); an optional failure continues, a required failure gets one bounded replan with the failure named; `GoalEvaluation` keeps ACTION_EXECUTED, EXECUTION_VERIFIED and GOAL_SATISFIED apart, COMPLETE only on GOAL_SATISFIED, the headline says which. `registry.find` no longer counts the vocative ("Zeus, …") as a subject and demotes FAILING capabilities; a single-capability call builds its payload from the input schema instead of `{"goal": …}`.

**Capabilities.** `manifest.health` (healthy / degraded / failing / unverified, consecutive failures, last error, calls, repairs) written by every execution in `CapabilityService.execute`, persisted, shown apart from `status` in the Capability Center and by the doctor. `runtime/paths.resolve_workspace_path`: one idempotent workspace path model with an existence check before dispatch (the `workspace\workspace\…` defect).

**Truthful health.** Doctor checks carry a subsystem; `overall` is HEALTHY / DEGRADED / FAILING from the important subsystems (core, voice, wakeword, knowledge, capabilities, projects); new functional checks `wakeword`, `knowledge`, `capability_health`, `projects`; `expert` reads the gateway's cached state (`AVAILABLE`, `QUOTA_EXHAUSTED`, `RATE_LIMITED`, `NOT_INSTALLED`, `NOT_AUTHENTICATED`, …; execution evidence outranks a probe; never PAYG). Live: DEGRADED while the stale listener had no report, HEALTHY after the supervisor relaunched it.

**Views.** Projects: owner projects first, acquisition attempts grouped per capability, constellation settles and stops (no perpetual CPU), labels avoid collisions. Mission Control: unified `/api/missions`, concise titles, PHASE / RESULT / DEPLOYMENT rendered apart, attempt families, seven filters, deep view for engine missions. Activity: request-centric groups with a goal verdict, repeated fragments collapse (`×5`). Knowledge: create / link / ingest. Diagnostics: subsystem cards, a voice block (listener, STT, wake model, evaluation, TTS, barge-in, ignored utterances). Headless-Chrome CDP sweep of all views: 0 console errors.

**Acceptance.** `tests/test_semantic_acceptance.py` (A–K, 14 tests) and `tests/test_wake_pipeline.py` (39). Full suite 1835 passed, 5 skipped, 1 xpassed. Live evidence: `data/acceptance_evidence/H1_hardening_live.json`.


## Phase 31 — the listening session (owner's live test failed after the wake fix) — FIXED, LIVE

Owner evidence: Voice Studio scores "Zeus" 1.000 at 0.55, yet the main interface filled with the literal "stopped" and no command was heard. Traced in the live logs: every wake called the listener's `_interrupt()` → `POST /api/stop` → `stop_current()` → NOTIFICATION "stopped" → `chat.js` printed it as a note (138 rows); and every wake ended "(too short - ignored)" because the natural pause between "Zeus" and the command exceeded the 0.9 s silence endpoint before any speech. Fix: `speech.listener.VoiceSession` + `CaptureLoop` — IDLE → WAKE_DETECTED → LISTENING (armed for 3.0 s, derived: detector confirms ~160 ms after the word, pre-roll 640 ms, human pause 0.3–2 s) → CAPTURING (speech heard; then the 0.9 s endpoint) → UTTERANCE_CAPTURED → SENT → IDLE; a session ends once with a reason code (`no_speech_after_wake`, `too_short`, `silence`, `max_length`, `sent`) and every transition is posted to `/api/voice/session` (DIAGNOSTIC events with the session id; the eye shows LISTENING). Stop semantics split: `/api/voice/interrupt` (barge-in) interrupts only speech/an answer in progress and is a silent no-op otherwise; `/api/stop` (owner) is idempotent, reason-coded, says what it stopped and never emits a bare "stopped"; `hear(answer=False)` returns to IDLE. Tests: `tests/test_voice_session.py` (13). Live: core `bee34eb`, listener relaunched by the supervisor on the new code, synthetic wake/interrupt/utterance through the API → 0 "stopped" rows; one listener, one worker; owner threshold 0.55 and model preserved. **The spoken test at the microphone is the owner's.**

## Phase 32 — Schach Analyse (new tool) — BUILT, LIVE-TESTED ON THIS MACHINE

`tools/chess_analysis/`: automatic board finder (nine evenly spaced Canny/Hough grid lines on both axes, least-squares refit, chequer contrast inside, *no* alternation outside — the last test separates a real board from the same board shifted by one square), the owner's Chessaru YOLO weights on the CPU (never the GPU) with test-time augmentation and a 3-frame per-cell vote (measured: without TTA both kings were missed on the owner's own screen board; with it 0.80/0.77), side to move from three sources with the reason shown (legality: the side in check must move; motion: the single legal move between frames names the mover; prior: start position; owner flip), FEN with castling rights only for pieces at home, local Stockfish (`D:\stockfish-windows-x86-64-avx2`) MultiPV 5 via python-chess, an always-on-top draggable panel top-left, a status file, `/api/tools/chess[/start|/stop]` and the "Schach Analyse" view. Tests: `tests/test_chess_analysis.py` (15; real Stockfish, real labelled boards ≥ 90 % cells). Live through the product: Start via API → overlay process; a real board screenshot on screen → board 631 px found, FEN = start position, white to move (prior), e4 +0.42 / d4 / Nf3 / e3 / c4 at depth 12–13, 398 ms per frame, engine 1.7 s; Stop via API → process gone. Not verified: a live chess site (the lichess page rendered blank in the test browser); the owner's real game is the next test.
