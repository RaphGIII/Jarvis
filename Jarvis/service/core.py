...
class JarvisCore:
    ...
    #: What the owner reads when the free intelligence cannot answer.  Fixed
    #: text, generated by nobody, costing nothing; the provider's own words
    #: (429, 503, "high demand", a timeout) go to Activity, never here.
    FREE_UNAVAILABLE_DE = ("Die kostenlose KI ist gerade ausgelastet. Du kannst es erneut versuchen oder für diese Anfrage "
                           "SMART verwenden.")
    FREE_UNAVAILABLE_EN = "The free AI is busy right now. You can try again or use SMART for this request."
    INTELLIGENCE_UNAVAILABLE_DE = "Das Denkmodell ist gerade nicht erreichbar. Versuch es gleich noch einmal oder wechsle den Modus."
    INTELLIGENCE_UNAVAILABLE_EN = "The reasoning provider is not reachable right now. Try again shortly or switch the mode."
    ...
    def _free_unavailable_message(self, reason: str = "") -> str:
        ...
        ``reason`` is accepted for the callers' sake and deliberately not
        shown: the owner gets one deterministic sentence, and SMART is named
        as the owner's choice, never taken for them.
        """

        return self.FREE_UNAVAILABLE_DE if self.language.startswith("de") else self.FREE_UNAVAILABLE_EN

    def _semantic_unavailable_message(self, reason: str = "", suggestion: str = "") -> str:
        """Outside FREE: the configured reasoning provider is not reachable.  Deterministic; the reason stays in Activity."""

        base = self.INTELLIGENCE_UNAVAILABLE_DE if self.language.startswith("de") else self.INTELLIGENCE_UNAVAILABLE_EN
        return base + (f" {suggestion.strip()}" if suggestion and suggestion.strip() else "")
    ...
    def _answer_conversationally(self, text: str, scope: str) -> None:
        ...
        try:
            ...
            self._deliver(("Die Antwort ist fehlgeschlagen. Bitte frag mich gleich noch einmal." if de
                           else "The answer failed. Please ask again in a moment."),
    ...
    def warm(self, *, speech: bool = True) -> threading.Thread:
        ...
        def run() -> None:
            ...
            # READY: the intelligence that answers the owner is wired.  That
            # is the gateway's reasoning route -- known from configuration
            # and credentials, generated on by nobody.  The legacy local
            # model used to be loaded here to answer "OK" (71 s cold); it no
            # longer answers the owner in any mode, so it is not loaded at
            # all.  A product with no reasoning provider configured is still
            # READY: it says so, deterministically, instead of guessing.
            try:
                status = self.gateway_status()
                roles = status.get("roles") or {}
                free = roles.get("reasoning.free") or {}
                cloud = bool(status.get("cloud_reasoning_available"))
                if free.get("configured"):
                    detail = f"reasoning.free: {free.get('provider')}/{free.get('model')}"
                elif cloud:
                    detail = "cloud reasoning configured"
                else:
                    detail = "no reasoning provider configured; deterministic answers only"
                self.lifecycle.mark("intelligence", True, detail)
                self._health_ok, self._health_detail, self._health_checked_at = cloud, detail, time.time()
                self.emit(EventType.DIAGNOSTIC, {"warming": f"intelligence ready: {detail}"})
            except Exception as exc:  # noqa: BLE001
                self.lifecycle.mark("intelligence", False, f"{type(exc).__name__}: {exc}"[:300])
                self.emit(EventType.DIAGNOSTIC, {"warming": f"intelligence unavailable: {exc}"})
    ...
    def _probe_health(self) -> None:
        """Can the owner be answered?  Read from the gateway; nothing is generated.

        The badge's question is "can it answer me".  What answers is the
        configured reasoning route, and whether it is reachable is known
        from configuration, credentials and the provider health the last
        real calls left behind.  This used to run a generation on the local
        4B model every two minutes; that model no longer answers the owner
        and is not loaded for a status light.
        """
        ...
        try:
            ...
            roles = status.get("roles") or {}
            free = roles.get("reasoning.free") or {}
            provider = str(free.get("provider") or "")
            health = str(((status.get("providers") or {}).get(provider) or {}).get("health") or "unknown")
            self._health_ok = bool(status.get("cloud_reasoning_available")) and health in {"ok", "unknown"}
            self._health_detail = (f"reasoning.free {provider}/{free.get('model')}: {health}" if free.get("configured")
                                   else "no cloud reasoning configured")
        except Exception as exc:
            self._health_ok = False
