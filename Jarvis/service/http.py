...
class JarvisHTTPServer:
    ...
    def handle_api(self, path: str, payload: dict[str, Any]) -> tuple[int, dict[str, Any]]:
        ...
        routes: dict[str, Callable[[dict[str, Any]], dict[str, Any]]] = {
            ...
            "/api/personality": lambda _: self.core.personality_view(),
            "/api/personality/preview": lambda body: self.core.personality_preview(dict(body.get("changes") or {})),
            "/api/personality/save": lambda body: self.core.personality_save(dict(body.get("changes") or {}),
                                                                             reason=str(body.get("reason", "personality edited")),
                                                                             authorization=str(body.get("authorization", ""))),
            "/api/personality/restore": lambda body: self.core.personality_restore(str(body.get("audit_id", ""))),
            "/api/conversation/rename": lambda body: self.core.conversation_rename(str(body.get("id", "")), str(body.get("title", ""))),
            "/api/conversation/pin": lambda body: self.core.conversation_pin(str(body.get("id", "")), bool(body.get("pinned", True))),
            "/api/conversation/assign": lambda body: self.core.conversation_assign(str(body.get("id", "")), str(body.get("project_id", ""))),
            "/api/conversations/search": lambda body: self.core.conversation_search(str(body.get("query", "")), limit=int(body.get("limit", 20) or 20)),
